...
// 下面这个函数是完成某个需要旋转的面的四个边的元素的旋转，5个元素中，
// 第一个是temp变量，保存中间结果，剩下的四个变量分别对应了这个面的四个边
static void line_rotate90(LINEITEM item[5]) // do line rotation 90 degree
{
    int i, j;
    // 5次循环，依次将后面的赋值给前面，最后将temp的值赋值给最后一个，达到旋转的效果
    for (i=0; i<5; i++) {
        int   dst_idx    = (i+0) % 5;
        int   src_idx    = (i+1) % 5;
        // 这里的buffer对应的就是每个边的第一个元素的地址
        char *dst_buf    = item[dst_idx].buffer;
        char *src_buf    = item[src_idx].buffer;
        // stride表示在这个元素对应的面中寻找在这个边中的下一个元素需要走的步长
        // 比如旋转正面时，还需要旋转它周围的四个边的元素，这四个边的元素分别在
        // l,d,r,u四个面上，左边记录的是[0][2]元素，那么它和正面相交的边的下一个元素在
        // 左面的位置就是[1][2]，相当于下一行，步长为3，如果是底面，记录第一个元素的地址[0][0]，下一个元素
        // 为[0][1]，步长为1，如果是右面，记录第一个元素[2][0]，下一个元素[1][0]，步长为-3，
        // 如果为上面的元素，记录元素[2][2]，下一个元素为[2][1]，步长为-1
...
// 指定一个面，将这个面的九个元素旋转90度，先拷贝一份，然后通过循环进行赋值，达到旋转的目的
...
static void cube_f(CUBE *c) // f operation
{
    char temp[3];
    LINEITEM lines[5] = {
        // temp用来保存中间结果，最后它的值赋给了最后一个元素，也就是u面
        { 1,  temp          },
        // 旋转正面时，需要旋转左面到上面，现将其保存到temp中，保存[0][2]元素的地址，
        // 需要旋转的下一个元素为[1][2]，步长为3
        { 3,  &(c->l[0][2]) },
        // d面保存了[0][0]地址，下一个元素为[0][1]，步长为1
        { 1,  &(c->d[0][0]) },
        // 右面保存[2][0]地址，下一个元素为[1][0]，步长为-3
        {-3,  &(c->r[2][0]) },
        // 上面保存[2][2]地址，下一个元素[2][1]，步长-1
        {-1,  &(c->u[2][2]) },
    };
    // 先将四个边的元素旋转90度
    line_rotate90(lines);
...
static CUBE* search(TABLE *table, CUBE *start, int state, char *oplist, int opnum, int cutval)
{
    ...
    while (table->close < table->open) {
        ...
        // dequeue a cube from open table，对当前这个cube循环执行如下的所有操作
        curcube = &(table->cubes[table->close++]);

        // extend cubes check state and put new cubes into open table
        // 遍历所有的操作，都是从当前curcube开始执行，都是先将其赋值给了newcube，而newcube又是指向了下一个新的open的位置
        for (i=0; i<opnum; i++) {
            // 从open中取出下一个cube,这里是地址引用
            newcube = &(table->cubes[table->open]);
            // 将curcube 赋值给newcube,相当于在下一个位置先保存了curcube，后面在对其进行操作
            memcpy (newcube, curcube, sizeof(CUBE));
            // 对新的cube执行操作变成新的cube保存在open的位置
            cube_op(newcube, oplist[i]);
            // 记录这次操作
            newcube->op     = oplist[i];
            // newcube 的父类就是curcube
            newcube->parent = curcube;
            // 从初始状态一层一层开始搜索，直到某一层不满足时，返回可以得到的分数
            newstate = cube_check_state(newcube, 0    );
            // 逐层计算,直到分数大于state时返回
            newvalue = cube_check_state(newcube, state);
            // 如果对curcube执行的操作到达的新的newcube的值大于指定的state，则表示状态找到
            if (newstate >= state) return newcube; // found
            // 如果新到达的状态值小于当前状态值，或者新的状态往上有四个相同的操作，则这个操作认为无效操作，继续循环下一次的操作
...
static void print_solve_oplist(CUBE *cube)
{
    ...
    // 下面的循环中，对多种操作进行了合并，比如有两次旋转90度，等于一个旋转180度
    while (cube) {
        if (cube->op >= 0) {
            if (last != cube->op) {
                // 表示之前有相关操作
                ...
                // 和上一次操作相同，则times++，这里最多加到2，因为不存在四次相同的操作，之前已经过滤掉了
                times++;
            }
        } else {
